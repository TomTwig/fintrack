...
export class FixedCostService {
  ...
  async loadAll(): Promise<void> {
    ...
    this.fixedCostsSubject.next(list);
  }
  ...
  /** Bucht einen Fixkosteneintrag manuell als Transaktion für heute */
  ...
  /** Prüft ob ein Fixkosteneintrag in diesem Monat bereits gebucht wurde */
  async isPaidThisMonth(fixedCostId: number): Promise<boolean> {
    ...
    const result = await this.db.getDb().query(
      `SELECT COUNT(*) AS count FROM transactions
       WHERE fixed_cost_id = ? AND date BETWEEN ? AND ?`,
      [fixedCostId, from, to],
    );
